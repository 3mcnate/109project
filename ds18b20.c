...
void ds_write1bit(void)
{
    ...
    DDRB |= (1 << PB4); // pull bus low
    _delay_us(2); // delay 2 usec to let sensor know we're writing
    DDRB &= ~(1 << PB4); // let bus float back up so sensor can sample high between 15-60 us
    _delay_us(60);
}
